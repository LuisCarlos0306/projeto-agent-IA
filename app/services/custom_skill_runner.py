...
def run_custom_skill(
    ...
    *,
    ...
    try:
        ...
        if failed:
            status = "attention"
            summary = f"Skill {skill['name']} concluída com {failed} comando(s) retornando erro."
        elif pending_scripts:
            status = "attention"
            summary = (
                f"Diagnóstico da Skill {skill['name']} concluído. "
                f"{len(pending_scripts)} script(s) permanecem aguardando aprovação para qualquer correção."
            )
        else:
            status = "healthy"
        ...
        return {
            ...
            "skill_id": skill_id,
            "name": skill["name"],
            "mode": skill.get("mode") or "read_only",
            "status": status,
            ...
            "approval_required": bool(pending_scripts),
            "executed_actions": [],
        }
    finally:
        executor.close()
